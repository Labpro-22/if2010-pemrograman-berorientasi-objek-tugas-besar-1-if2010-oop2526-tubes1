...
void GameScreen::drawBoard()
{
    ...
    for (int i = 0; i < 40; i++)
    {
        ...
        // 1. HIGHLIGHT TILE yang didarati player aktif atau sedang dituju
        ...
        if (activeP >= 0 && activeP < (int)playerVisuals.size())
        {
            if (i == (int)playerVisuals[activeP].targetTileIdx)
            {
                ...
                DrawRectangleRec(r, {255, 255, 0, (unsigned char)(50 + 30 * sinf(glowTimer * 6))});
            }
        }

        float rot = (TILE_DEFS[i].side == "BOTTOM") ? 0 : (TILE_DEFS[i].side == "LEFT") ? 270
    ...
    // 2. DRAW PION (Berdasarkan posisi visual/animasi, bukan posisi logis)
    for (int p = 0; p < (int)playerVisuals.size(); p++)
    {
        float visualIdx = playerVisuals[p].currentTileIdx;

        // Interpolasi posisi antara tile A dan tile B
        int tileA = (int)visualIdx;
        int tileB = (tileA + 1) % 40;
        float frac = visualIdx - (float)tileA;

        Vector2 posA = getTileCenter(tileA);
        Vector2 posB = getTileCenter(tileB);

        Vector2 drawPos = {
            posA.x + (posB.x - posA.x) * frac,
            posA.y + (posB.y - posA.y) * frac};

        // Offset sedikit agar pion tidak tumpang tindih sempurna jika di tile yang sama
        float offset = (float)p * 4.0f;

        DrawCircleV(drawPos, 10, WHITE);          // Border
        DrawCircleV(drawPos, 8, playerColors[p]); // Pion
        DrawText(std::to_string(p + 1).c_str(), (int)drawPos.x - 3, (int)drawPos.y - 5, 10, WHITE);
    }
