...
private:
    ...
    struct TradeSaveData
    {
        int proposerId = -1;
        std::string proposerUsername;
        ...
        std::vector<std::string> offeredPropertyCodes;
        std::vector<std::string> requestedPropertyCodes;
    };

    static TurnManagerSaveData parseTurnManager(const std::vector<std::string>& lines);
    static std::vector<TradeSaveData> parseTradeManager(const std::vector<std::string>& lines);

    static class PropertyTile* findPropertyByCode(Game& game, const std::string& code);

    static void applyTurnManager(Game& game, const TurnManagerSaveData& turnManagerData);
    static void applyTradeManager(Game& game, const std::vector<TradeSaveData>& tradesData);
};
