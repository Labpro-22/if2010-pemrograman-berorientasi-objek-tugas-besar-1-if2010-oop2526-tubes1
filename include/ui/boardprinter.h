    ...
    cout << col
              ...
              << ("[" + colorGroupToString(
                     prop->type() == PropertyType::STREET
                       ? static_cast<Street*>(prop)->colorGroup()
                       : ColorGroup::DEFAULT)
                  + "] " + prop->name() + " (" + prop->code() + ")")
              << " |\n"
              << "+================================+\n"
...
void BoardPrinter::printProperties() const {
    ...
    for (auto* prop : player.properties()) {
        string group;
        if (prop->type() == PropertyType::STREET)
            group = colorGroupToString(static_cast<Street*>(prop)->colorGroup());
        else if (prop->type() == PropertyType::RAILROAD)
            group = "STASIUN";
        else
            group = "UTILITAS";
